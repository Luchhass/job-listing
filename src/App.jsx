  ...
  return (
    <>
      <header className="header">
        <ListingFilter filter={filter} deleteFilter={deleteFilter} fullDelete={fullDelete}/>
      </header>
        ...
        <div className="filterContent">
          {filter.map((x, i) => (
            <div className="textFilter">
              <p key={i}>{x}</p>
              <button onClick={() => deleteFilter(x)}> <img src="./assets/images/devjobs-cross.png" alt="" /> </button>
            </div>
          ))}
        </div>
...
function JobItem({company, logo, new: isNew, featured, position, postedAt, contract, location, tags, addFilter}) {
  return (
    <div className={featured ? "jobItem featuredJob" : "jobItem"}>
      <JobDetails company={company} logo={logo} isNew={isNew} featured={featured} position={position} postedAt={postedAt} contract={contract} location={location}/>

      <div className="jobTags">
        {tags.map((x, i) => (
          <a href="#" key={i} onClick={(e) => {e.preventDefault(); addFilter(x);}}> {x} </a>
        ))}
      </div>
    </div>
  );
...
function JobDetails({company, logo, isNew, featured, position, postedAt, contract, location}) {
  return (
    <div className="jobDetails">
      <a href="#" className="companyLogo"><img src={logo} alt={company} /></a>
      
      <h4><a href="#">{company} {isNew && <span className="new">new!</span>}{featured && <span className="featured">featured</span>}</a></h4>
